...
import com.example.hospimanagementapp.util.ValidationManager; // NHS number validator
...
public class PatientRegistrationActivity extends AppCompatActivity { // Screen to capture and save a patient
    ...
    private void savePatient() {
        ...
        if (!ValidationManager.validateNhsNumber(nhs)) {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            try {
                ...
                p.fullName = CryptoManager.encrypt(name);
                p.dateOfBirth = CryptoManager.encrypt(dob); // Consider normalising/validating format upstream
                p.phone = CryptoManager.encrypt(phone);
                p.email = CryptoManager.encrypt(email);

                Log.d("ENCRYPT_TEST", "Encrypted name = " + p.fullName);
                Log.d("ENCRYPT_TEST", "Encrypted email = " + p.email);
