...
public class PatientSummaryActivity extends AppCompatActivity {
    ...
    private void loadRecord() {
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            runOnUiThread(() -> {
                if (record != null) {
                    String conditions = CryptoManager.safeDisplay(record.conditions);
                    String allergies = CryptoManager.safeDisplay(record.allergies);
                    String medications = CryptoManager.safeDisplay(record.medications);

                    tvConditions.setText("Conditions: " + conditions);
